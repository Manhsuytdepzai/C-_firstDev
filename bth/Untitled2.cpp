...
void nhap_sv(sv a[], int n)
{
	for (int i=0; i<n; i++)
	{
		...
		cout<<"gioi tinh ( nam : 0, nu : 1 ) : ";cin>>a[i].gioi_tinh;
...
void in_sv(sv a[], int n)
{
	...
	cout<<setw(20)<<a[i].year<<"|"<<setw(20)<<sex(a,n)<<"|";
...
void tbcnnu_max(sv a[], int n)
{
	...
	int vitri=0;
    for(int i=1;i<n;i++)
        if( a[i].gioi_tinh == 0 && max < a[i].tbcn)
		{
            max = a[i].tbcn;
            vitri =i;
        }
...
void them_sv(sv a[], int n) {
	...
	cout<<"nhap thong tin sinh vien muon them : "<<endl;
		...
		cout<<"gioi tinh : ";cin>>them.gioi_tinh;
...
void xoa_sv(sv a[], int n) 
{
	...
	cin.ignore();
	cout<<"ma sinh vien : ";cin.getline(xoa.masv, 20); 
	int x;
	bool y = false;
	for(int i = 0 ; i < n; i++) 
	{
		if(a[i].masv > xoa.masv && a[i + 1].masv <=xoa.masv) 
		{
			x = i + 1;
			y = true;
		}
