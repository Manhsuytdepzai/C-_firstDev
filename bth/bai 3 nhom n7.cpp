...
int main() {
	...
	delta=b*b-4*a*(c-d);
	float X1=float((-b-sqrt(delta))/2.0*a);
	float X2=float((-b+sqrt(delta))/2.0*a); 
	cout<<"	Giai phuong trinh bac 2 : "<<a<<"X^2 + "<<b<<"X + "<<c-d<<" = "<<0<<endl;
		if (delta==0)
			{
				if(a<0)
				{
					cout<<"	Tap nghiem cua BPT la rong"<<endl;
				}
			else
				{
					cout<<"	Tap  nghiem cua phuong trinh la R / {"<<-b/2.0*a<<"}"<<endl;
				}
			}	
		if (delta<0)
			{
		...
		if (delta>0)
				...
				cout<<"	Pt co 2 nghiem phan biet x1 = "<<X1<<" ; x2 = "<<X2<<endl;	
			{
				if(a<0)
				{
					cout<<"	Tap  nghiem cua phuong trinh la : { X < "<<X2<<" } U { X > "<<X1<<" }"<<endl;
				}
			else
				{
					cout<<"	Tap  nghiem cua phuong trinh la : {"<<X1<<" ; "<<X2<<"}"<<endl;
				}
			}
			}	
	return 0; 
